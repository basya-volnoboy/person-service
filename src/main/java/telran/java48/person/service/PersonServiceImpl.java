...
public class PersonServiceImpl implements PersonService, CommandLineRunner {
	...
	public PersonDto findPersonById(Integer id) {
		...
		//checkPersonstype(person);
		
		return modelMapper.map(person, PersonDto.class);
	}

	private Object checkPersonstype(Person person) {
	...
	public PersonDto updatePersonName(Integer id, String name) {
		Person person = personRepository.findById(id).orElseThrow(PersonNotFoundException::new);
		person.setName(name);
	//	personRepository.save(person); //если метод транзакционный то хайбер нет сам все сохранит (коммит)
		return modelMapper.map(person, PersonDto.class);
	}
	...
	public Iterable<PersonDto> findPersonsByName(String name) {
		return 
				personRepository.findByNameIgnoreCase(name)
				.map(p -> modelMapper.map(p, PersonDto.class))
				.collect(Collectors.toList());
	}
	...
	public Iterable<PersonDto> findPersonsBetweenAge(Integer minAge, Integer maxAge) {
		...
		return 
				personRepository.findByBirthDateBetween(from, to)
	...
	public void run(String... args) throws Exception {
		if(personRepository.count() == 0) {
			...
			//Child child1 = new Child(3000, "Moshe", LocalDate.of(2018, 7, 5), new Address("Ashkelon", "Bar Kohva", 21), "Shalom");
			personRepository.save(person);
